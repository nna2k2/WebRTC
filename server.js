...
io.sockets.on('connection', function (socket) {
     ...
     */
    socket.on('create or join', (room,displayName) => {
        log('Create or Join room: ' + room);
        if (!rooms[room]) {
            rooms[room] = [];
        }
        rooms[room].push({ id: socket.id, displayName: displayName });
    ...
    socket.on('leave room', (room,displayName) => {
        socket.leave(room);
        ...
        io.in(room).emit('userLeft',socket.id);
    });

    /**
     * When participants send messages
     */

    socket.on('message_chat', (displayName,message,room) => {
        console.log(displayName+'đã chat')
        // Ensure that data is defined and contains the expected properties
        if (displayName && message) {
            // Broadcast the message and sender's name to all clients
            if (displayName && message && room) {
                // Broadcast the message and sender's name to all clients in the same room
                socket.broadcast.to(room).emit('message_chat', displayName, message);
            }
        }
    });
