...
class Webrtc extends EventTarget {
    ...
    joinRoom(room, displayName) {
        ...
        this.socket.emit('create or join', room, displayName);
    }
    ...
    getLocalScreenStream() {
        return navigator.mediaDevices
            ...
            .then((stream) => {
                this.log('Got local screen stream.');
                this._localStream = stream;
                return stream;
            })
            .catch(() => {
                this.error("Can't get display media");

                this._emit('error', {
                    error: new Error(`Can't get display media`),
                });
            });
    }
    ...
    _onSocketListeners() {
        ...
        this.socket.on('created', (room, socketId) => {
            ...
            this._isAdmin = true;
            console.log("SSfjknjasdnsjadnjadsnjasdnjkdas" + this._myId)
        ...
        this.socket.on('message_chat', (displayName, message) => {
            console.log('_sendMessageData')
            this._sendMessageData(displayName, message);
        });
        // Send list user in room
        this.socket.on('userJoined', (usersRoom) => {
            console.log('Sent users')
            this._sendListUsersRoom(usersRoom);
        });
        // Send list user in room
        this.socket.on('userLeft', (userId) => {
            console.log('UserLeft')
            this._sendUserLeft(userId);
        });
    ...
    _sendMessageData(name, message) {
        if (name.trim() !== '' && message.trim() !== '') {
            // Send the message and sender's name to the server
            this._emit('message_chat', {name, message});
            message = '';
        }

    }

    _sendListUsersRoom(usersRoom) {
        this._emit('userJoined', {usersRoom: usersRoom});
    }

    _sendUserLeft(userId) {
        this._emit('userLeft', {userId: userId});
    }
    ...
    sendMessageData(displayName, message, roomId) {
        console.log('sendMessageData')
        this.socket.emit('message_chat', displayName, message, roomId);
    }
// In webrtc.js
    _createAndSendOffer(socketId) {
        this.pcs[socketId].createOffer()
            .then((offer) => {
                return this.pcs[socketId].setLocalDescription(offer);
            })
            .then(() => {
                this._sendMessage({
                    type: 'offer',
                    sdp: this.pcs[socketId].localDescription,
                }, null, this.room);
            })
            .catch((error) => {
